...
interface FamilyPaymentProps {
  onBack: () => void;
  onSwitchToPremium: () => void;
  onPaymentComplete: () => void;
}

export const FamilyPayment: React.FC<FamilyPaymentProps> = ({ onBack, onSwitchToPremium, onPaymentComplete }) => {
  const { t } = useLanguage();
    ...
    <div className="w-full max-w-[412px] mx-auto flex flex-col items-center min-h-screen justify-center px-4">
      <div className="w-full flex flex-col items-center bg-gradient-to-b from-transparent to-black/10 rounded-2xl p-6 backdrop-blur-sm">
        ...
        <div className="w-full max-w-[300px] bg-white rounded-[15px] p-6 mb-8 text-center">
          <h2 className="text-[#010177] text-[32px] font-bold mb-4">{t.familyPlan}</h2>
          <div className="text-[#007] text-[32px] font-bold mb-6">
            <span>19,</span>
            <span className="text-2xl">90</span>
            <span>/mês</span>
          </div>
          <div className="text-[#010177] text-xl font-medium mb-6">•{t.upToThreePeople}</div>
          ...
          <div className="space-y-4 text-left text-gray-700">
            ...
            <div className="flex items-center">
              <span className="text-green-500 mr-2">✓</span>
              <span>Até 3 perfis de usuário</span>
            </div>
            <div className="flex items-center">
              <span className="text-green-500 mr-2">✓</span>
              <span>Compartilhamento de contatos de emergência</span>
            </div>
            <div className="flex items-center">
              <span className="text-green-500 mr-2">✓</span>
              <span>Relatórios familiares</span>
            </div>
          </div>
        </div>
        ...
        <div className="flex flex-col items-center gap-4 w-full">
          ...
          <LinkText href="#" onClick={onSwitchToPremium} className="text-center block text-white/80 hover:text-white transition-colors">
